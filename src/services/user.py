...
from schemas.user import UpdateUserSchema, GetUserSchema
...
class UserService:
    ...
    async def update_user(self, user_id: int, user_data: UpdateUserSchema) -> GetUserSchema:
        """
        ...
        Returns:
            GetUserSchema: Обновленные данные пользователя
    ...
    async def search_users(
        ...
        Returns:
            Page[GetUserSchema]: Пагинированные результаты поиска
        """
        users_page = await self.user_repository.search_users(
            query=query,
            current_user_id=current_user_id,
            params=params
        )
        items = [GetUserSchema.model_validate(user) for user in users_page.items]
        return Page.create(
            items=items,
